...
module.exports = {
    async createCharacter(req, res, next) {
        try {
            ...
            const allClass = await characterModel.getAllClasses()
            var specifiedClass;
            ...
            const characterId = await characterModel.createCharacter(user_id, character_name, character_class, lvl, weapon, helmet, chestplate, leggings, hp, atk, atk_spd, movement_spd, energy, mana, passive, skill1, skill2, pet_name);

            const createdCharacter = await characterModel.getCharacterWithCharacterId(characterId)

            const faction = createdCharacter[0].faction
            responseView.confirmCreated(res, { id: characterId, user_id, character_name, character_class, faction: faction, lvl, weapon, helmet, chestplate, leggings, pet_name, pet_lvl: lvl });
            next()
        } catch (err) {
    ...
    async checkCharacterExistence(req, res, next) {
        try {
            ...
            if(character_nonExistence) {
                responseView.NotFound(res, 'User does not have any character')
                return;
            }
    ...
    async petEvolve(req, res) {
        try {
            ...
            if (currentEvolution == 1 || currentEvolution == 2) {
                ...
                responseView.Forbidden(res, 'Pet is already fully evolved');
                return;
            }
