...
module.exports = {
    async createCharacter(user_id, character_name, character_class, lvl, weapon, helmet, chestplate, leggings, hp, atk, atk_spd, movement_spd, energy, mana, passive, skill1, skill2, pet_name) {
        const [result] = await db.query(`INSERT INTO characters (user_id, character_name, character_class, faction, lvl, weapon, helmet, chestplate, leggings, hp, atk, atk_spd, movement_spd, energy, mana, passive, skill1, skill2, pet_name, pet_lvl) VALUES (?, ?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [user_id, character_name, character_class, lvl, weapon, helmet, chestplate, leggings, hp, atk, atk_spd, movement_spd, energy, mana, passive, skill1, skill2, pet_name, lvl]);
        return result.insertId
    },

    async getCharacterWithUserId(user_id) {
        const [result] = await db.query('SELECT character_id, character_name, character_class, faction, lvl, weapon, helmet, chestplate, leggings, pet_name, pet_lvl FROM characters where user_id = ?', [user_id])
        return result
    },
